...
Download_caddy(){
	...
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/caddy_v1.0.4_linux_amd64.tar.gz"
	elif [[ ${bit} == "i386" || ${bit} == "i686" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/caddy_v1.0.4_linux_386.tar.gz"
	elif [[ ${bit} == "armv7l" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/caddy_v1.0.4_linux_arm7.tar.gz"
	elif [[ ${bit} == "arm64" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/caddy_v1.0.4_linux_arm64.tar.gz"
	else
...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/caddy_centos.txt -O /etc/init.d/caddy; then
		...
		if ! wget --no-check-certificate https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/caddy_debian.txt -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
