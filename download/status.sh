...
github_prefix="https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/"
...
Download_Server_Status_server() {
  cd "/tmp" || exit 1
  [[ ${mirror_num} == 2 ]] && bundle_link="https://cokemine.coding.net/p/hotarunet/d/ServerStatus-Hotaru/git/archive/master/?download=true" || bundle_link="https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/ServerStatus-Hotaru-master.zip"
  [[ ${mirror_num} == 2 ]] && github_link="https://hub.fastgit.org" || github_link="https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download"
  ...
  if [[ -e "${server_file}/sergate" ]]; then
    ...
    mv "/tmp/ServerStatus-Hotaru-master/server/sergate" "${server_file}/sergate"
    wget -N --no-check-certificate "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/hotaru-theme.zip"
...
Download_Server_Status_client() {
  cd "/tmp" || exit 1
  wget -N --no-check-certificate "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/status-client.py"
...
Download_Server_Status_Service() {
  ...
  if [[ ${release} == "archlinux" ]]; then
    wget --no-check-certificate "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/status-${mode}.service" -O "/usr/lib/systemd/system/status-${mode}.service" ||
      {
    ...
    wget --no-check-certificate "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/server_status_${mode}_${release}.txt" -O "/etc/init.d/status-${mode}" ||
      {
        echo -e "${Error} ServerStatus ${service_note}服务管理脚本下载失败 !"
        exit 1
      }
...
Install_vnStat() {
  ...
  cd "/tmp" || return 1
  wget --no-check-certificate https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/vnstat-latest.tar.gz
...
Install_jq() {
  ...
  if [[ ! -e ${jq_file} ]]; then
    if [[ ${bit} == "x86_64" ]]; then
      jq_file="${file}/jq"
      wget --no-check-certificate "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/jq-linux64" -O ${jq_file}
    elif [[ ${bit} == "i386" ]]; then
      jq_file="${file}/jq"
      wget --no-check-certificate "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/jq-linux32" -O ${jq_file}
    else
  ...
  if [[ ! -e ${region_json} ]]; then
    wget --no-check-certificate "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/zh.json" -O ${region_json}
  ...
  if [[ "${caddy_yn}" == [Yy] ]]; then
    ...
    [[ ! -e /usr/bin/caddy ]] && {
      ...
      if [[ ${release} == "debian" ]]; then
        ...
        curl -1sLf 'https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/gpg.key' | gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg
        curl -1sLf 'https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/debian.deb.txt' | tee /etc/apt/sources.list.d/caddy-stable.list
...
Update_Shell() {
  Set_Mirror
  sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/status.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1)
  ...
  wget -N --no-check-certificate "https://github.com/Xun-X/ServerStatusFloat/raw/refs/heads/main/download/status.sh" && chmod +x status.sh
...
Set_Mirror() {
  echo -e "${Info} 请输入要选择的下载源，默认使用自建，选择Coding.net不保证可用，注意：不建议将服务端部署在中国大陆主机上
